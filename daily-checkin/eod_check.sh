...
#    (e.g. "--> this is blocked, move to friday"), updating task files and
#    cleaning up the dailyLog accordingly.
...
2. Look for inline annotations on task lines -- text appended after the standard '- [ ] title -- duration -- hard/soft -- deadline' (or plain description) format that indicates Becca wants something changed, e.g. '--> this is blocked, move to friday', '--> push to next week', '--> change domain to admin', '--> actually due monday', etc. Ignore lines already in standard format with no extra annotation, and ignore Obsidian completion markers like '✅ YYYY-MM-DD'.

3. For each annotation found:
   - Find the matching task file in \"$VAULT/tasks/\" (match by title -- same fuzzy matching as sync_dailylog.py: the task's title appears in, or contains, the dailyLog line's title portion).
